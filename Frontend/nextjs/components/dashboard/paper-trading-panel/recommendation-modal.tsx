    ...
    return (
      <div className={styles.summaryGrid}>
        ...
        <div className={styles.summaryCard}>
          ...
          <span className={styles.summaryValue}>
            {controller.effectiveQuantity.toFixed(4)}
          </span>
        </div>
        <div className={styles.summaryCard}>
        ...
        <div className={styles.summaryCard}>
          ...
          <span className={styles.summaryValue}>
            {recommendation.spread != null
              ? formatPrice(recommendation.spread)
              : "-"}
          </span>
        </div>
        <div className={styles.summaryCard}>
  ...
  return (
      ...
      footer={
        <Space>
          <Button
            ...
            onClick={controller.closeRecommendationModal}
          >
            Close
          </Button>
          <Button
    ...
    >
      {controller.isRecommendationLoading ? (
        ...
        <div className={styles.feedbackBody}>
          <Alert
            type={controller.recommendationTone}
            showIcon
            title={recommendation.headline}
            description={recommendation.summary}
          />
          <div className={styles.feedbackMeta}>
            <Tag color={getRiskTone(recommendation.riskLevel)}>
              {recommendation.riskLevel} risk
            </Tag>
            <Tag color={getRecommendationActionTone(recommendation.recommendedAction)}>
          ...
          <div className={styles.feedbackBlock}>
            ...
            <div className={styles.feedbackMeta}>
              <Tag color="blue">Sentiment {recommendation.newsSentiment || "Neutral"}</Tag>
              {recommendation.newsImpactScore != null ? (
                <Tag color="gold">Impact {recommendation.newsImpactScore.toFixed(1)}</Tag>
              ) : null}
              {recommendation.newsRecommendedAction ? (
                <Tag color={getRecommendationActionTone(recommendation.newsRecommendedAction)}>
          ...
          <div className={styles.feedbackBlock}>
            <span className={styles.feedbackLabel}>Economic calendar</span>
            <Typography.Paragraph className={styles.helper}>
              {recommendation.economicCalendarSummary ||
                "No nearby high-impact macro event is currently shading this setup."}
            </Typography.Paragraph>
            <div className={styles.feedbackMeta}>
              {recommendation.economicCalendarRiskScore != null ? (
                <Tag color="orange">
                  Macro risk {recommendation.economicCalendarRiskScore.toFixed(1)}
                </Tag>
              ) : null}
              {recommendation.economicCalendarNextEventAtUtc ? (
                <Tag color="gold">
                  Next event {formatDateTime(recommendation.economicCalendarNextEventAtUtc)}
                </Tag>
              ) : null}
            </div>
            {recommendation.economicCalendarEvents.length > 0 ? (
              <ul className={styles.feedbackList}>
                {recommendation.economicCalendarEvents.map((item) => (
                  <li key={`${item.title}-${item.occursAtUtc}`}>
                    {item.title} · {formatDateTime(item.occursAtUtc)} · impact{" "}
                    {item.impactScore.toFixed(0)} · {item.source}
                  </li>
                ))}
              </ul>
            ) : null}
